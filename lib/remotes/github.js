...
function GitHub(options) {
  ...
  options.githubApi = options.githubApi || GITHUB_API_DEFAULT;
...
GitHub.prototype.getHash =function* (repo, ref) {
  var uri = options.githubApi + '/repos/' + repo + '/commits?sha=' + ref;
  debug('GET "%s"', uri);
  API_COUNTER++;
  var hash = null;
...
GitHub.prototype._versions = function* (repo) {
  var uri = options.githubApi + '/repos/' + repo + '/tags';
  debug('GET "%s"', uri);
  API_COUNTER++;
  var res = yield* this.request(uri, true);
...
GitHub.prototype._tree = function* (repo, ref) {
  var uri = options.githubApi + '/repos/' + repo + '/git/trees/' + ref + '?recursive=1'
  debug('GET "%s"', uri);
  API_COUNTER++;
  var res = yield* this.request(uri, true)
...
GitHub.prototype.archive = function (repo, ref) {
  console.log('using ' + options.githubApi);
  // http://developer.github.com/v3/repos/contents/#get-archive-link
  var root = options.githubApi + '/repos/' + repo;
